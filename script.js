...
document.addEventListener('DOMContentLoaded', () => {
    // เปลี่ยนจาก "barcodeInput" เป็น "manualBarcode" ให้ตรงกับ HTML
...
async function searchAndAddProduct(partialCode) {
    ...
    let allProducts = await db.products.toArray();// 1. ดึงข้อมูลสินค้าทั้งหมดออกมา (Dexie เร็วมากครับ)
    let found = allProducts.filter(p => // 2. กรองเฉพาะรายการที่รหัสลงท้ายด้วยตัวเลขที่พิมพ์  เราใช้ .toString() เพื่อให้มั่นใจว่าเปรียบเทียบเป็นข้อความได้แม่นยำ
        p.code && p.code.toString().endsWith(partialCode)
    );
    if (found.length === 1) {// 3. จัดการผลลัพธ์
        // เจอตัวเดียวเป๊ะๆ เพิ่มลงตะกร้าทันที
        onScanSuccess(found[0].code);
    } else if (found.length > 1) {
        // เจอหลายตัว ให้แจ้งศิษย์พี่ทราบ (ป้องกันการหยิบผิดตัว)
...
let previewTimer = null; // เพิ่มตัวแปร global ไว้ด้านบนสุดของไฟล์ script.js ถ้ายังไม่มี
async function onScanSuccess(d) {
    ...
    const overlay = document.getElementById("scan-preview-overlay"); // 🌟 ระบบ Preview แบบใหม่
    ...
    updateBarcodeTitle();// 🌟 อัปเดตอิโมจิให้สนุกสนานทุกครั้งที่สแกน
